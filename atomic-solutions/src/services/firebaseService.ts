...
const isInvalidValue = (val: any) => !val || val === '' || val.includes('MY_') || val.includes('YOUR_');
...
const getFirebaseConfig = () => {
  ...
  // If env variables are missing or invalid, we check for other sources
  if (isInvalidValue(envConfig.apiKey) || isInvalidValue(envConfig.projectId)) {
    try {
      // @ts-ignore
      const configStr = import.meta.env.VITE_FIREBASE_CONFIG;
      if (configStr && configStr !== '{}') {
        const config = JSON.parse(configStr);
        if (config.apiKey && !isInvalidValue(config.apiKey)) {
          console.log("Using injected VITE_FIREBASE_CONFIG");
          return config;
        }
      }
    } catch (e) {
    ...
    // Try finding the config manually in the global scope if injected by the platform
    // @ts-ignore
    if (window.__FIREBASE_CONFIG__ && !isInvalidValue(window.__FIREBASE_CONFIG__.apiKey)) {
  ...
  // Final check: if it's still placeholder or missing, return null
  if (isInvalidValue(envConfig.apiKey) || isInvalidValue(envConfig.projectId)) {
    console.warn("NO VALID FIREBASE CONFIG FOUND. Entering Mock Mode.");
    return null;
  }

  console.log("Using Firebase Config for project:", envConfig.projectId);
  return envConfig;
};

// We will resolve config dynamically to avoid build errors if the file is missing
