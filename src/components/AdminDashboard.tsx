...
By: *Atomic Solutions*`;
  ...
  const deleteUser = async () => {
    ...
    const targetId = userToDelete.uid || (userToDelete as any).id;
    if (!targetId) {
    ...
    try {
      ...
      await dataService.deleteDoc('users', targetId);
    } catch (e: any) {
      console.warn("Direct Firestore delete failed, trying fallback:", e);
      try {
        // 2. Fallback: Soft-delete in database
        await dataService.updateDoc('users', targetId, { isDeleted: true, isBlocked: true });
      } catch (err2: any) {
        console.warn("Database update also restricted:", err2);
      }
    } finally {
      // Always remove from local state so UI updates immediately
      setUsers(prev => prev.filter(u => u.uid !== targetId && (u as any).id !== targetId));
        ...
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-100 p-4 md:px-8 md:py-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="flex items-center gap-2">
               ...
               <Button 
                 variant="ghost" 
                 className="group flex items-center gap-2 bg-teal/10 border border-teal/20 text-teal hover:bg-teal hover:text-navy px-4 py-2.5 rounded-2xl shadow-sm transition-all"
                 onClick={() => {
                   if (!viewAsCustomer) toggleAdminView();
                   navigate('/');
                 }}
                 title="Preview live website as a customer"
                >
                   <Eye size={18} className="text-teal group-hover:text-navy transition-colors" />
                   <span className="text-[10px] font-black uppercase tracking-widest hidden md:block">Preview Website</span>
                </Button>
            </div>
          </div>
        </header>
