...
const StaffDashboard: React.FC = () => {
  ...
  if (isPendingStaff) {
    return (
      <div className="min-h-screen bg-slate-app flex flex-col items-center justify-center p-6 text-center font-sans">
        <div className="max-w-md w-full animate-in fade-in zoom-in duration-500">
          <div className="w-24 h-24 bg-orange-100 rounded-[32px] flex items-center justify-center mx-auto mb-8 text-orange-600 shadow-xl shadow-orange-500/10">
            <Clock size={40} />
          </div>
          <h1 className="text-3xl font-black text-navy uppercase tracking-tighter mb-4">Under Review</h1>
          <p className="text-gray-500 font-medium leading-relaxed mb-8 text-sm">
            Your application to join as a Service Professional is currently pending approval from our administration team. We will notify you once your account is activated.
          </p>
          <Button 
            variant="ghost" 
            onClick={logout}
  ...
  useEffect(() => {
    ...
    const unsubscribeBookings = dataService.subscribe('bookings', (allBookings) => {
      ...
      // Show bookings assigned to this staff member OR incoming customer bookings waiting for technicians
      const staffBookings = allBookings.filter((b: Booking) => {
        // 1. Explicitly assigned to this staff member
        if (b.staffId === user.uid) return true;

        // 2. Incoming customer bookings that are unassigned or pending
        const isIncoming = (!b.staffId || b.staffId === '' || b.status === 'Pending');
        if (isIncoming && b.status !== 'Rejected' && b.status !== 'Completed') {
          if (profile?.staffCategory) {
            const cat = profile.staffCategory.toLowerCase();
            const firstWord = cat.split(/[\s(/)]+/)[0];
            const sName = (b.serviceName || '').toLowerCase();
            const sCat = (b.serviceCategory || '').toLowerCase();
            const catField = (b.category || '').toLowerCase();
            const sub = (b.subCategory || '').toLowerCase();
            return sName.includes(firstWord) || sCat.includes(firstWord) || catField.includes(firstWord) || sub.includes(firstWord);
          }
          return true;
        }
        return false;
      ...
      if (prevBookingsCount.current !== null && staffBookings.length > prevBookingsCount.current) {
        const newJob = staffBookings[0];
        toast(`New Customer Booking!`, {
          description: `New ${newJob.serviceName} booking from ${newJob.userName}.`,
          icon: <Briefcase className="text-teal" size={18} />,
          action: {
            label: 'Open Details',
    ...
    const unsubscribeNotifications = dataService.subscribe('notifications', (allNotifs) => {
      ...
      if (prevNotifsCount.current !== null && staffNotifs.length > prevNotifsCount.current) {
        const newNotif = staffNotifs[0];
        if (!newNotif.read) {
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-2">
              ...
              <div className={cn(
                ...
                connectionStatus === 'online' ? "bg-white/10 border-white/20 text-white" :
                connectionStatus === 'syncing' ? "bg-teal/20 border-teal/30 text-teal animate-pulse" :
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center justify-between px-4">
                      <h2 className="text-xl font-black text-navy uppercase tracking-tight flex items-center gap-3">
                        <Clock className="text-teal" size={24} /> Incoming &amp; Assigned Bookings
                      </h2>
                    </div>
                    
                    {bookings.filter(b => b.status === 'Assigned' || b.status === 'Pending').length === 0 ? (
                      <div className="bg-white rounded-[32px] p-12 border-2 border-dashed border-gray-100 text-center">
                        <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4 text-gray-300">
                          <Briefcase size={32} />
                        </div>
                        <p className="text-gray-400 font-bold uppercase text-[10px] tracking-widest">No new customer bookings right now</p>
                      </div>
                    ) : (
                      <div className="grid grid-cols-1 gap-4">
                        {bookings.filter(b => b.status === 'Assigned' || b.status === 'Pending').map(booking => (
                          <JobCard 
                            key={booking.id} 
                            booking={booking} 
